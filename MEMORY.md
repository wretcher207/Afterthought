...
- **Capture failure leaves orphan sessions:** ✅ FIXED 2026-06-08. `CaptureEngine` now exposes `onPermissionDenied` callback; `AppState` sets it to call `stopSession()`, so a denied permission properly ends the session (sets `endedAt`, nils `activeSession`) instead of leaving an unterminated row.
- **Screenshot thumbnails:** ✅ DONE 2026-06-08. `EntryRowView` now renders the stored HEIC thumbnail for screenshot entries via `ScreenshotThumbnail` (async disk load, camera icon placeholder during fetch). Notes and other entry kinds retain the SF Symbol icon.
